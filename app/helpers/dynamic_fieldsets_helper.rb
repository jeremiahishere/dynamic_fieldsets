...
#    other_fsas_values = DynamicFieldsets::FieldsetAssociator.where('fieldset_id != ?',fieldset.id).all.collect(&:field_values)#.flatten
#
#    children = []
#    fieldset.fieldset_children.each do |child_field|
#      value = values[child_field.id]
#      dependency_group = DynamicFieldsets::DependencyGroup.where(:fieldset_child_id => child_field.id).first
#      if dependency_group.nil?
#        children << child_field.child
#      else
#        dependencies = dependency_group.dependency_clauses.collect(&:dependencies).flatten.uniq
